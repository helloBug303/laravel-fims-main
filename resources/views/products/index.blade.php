            ...
            <div class="row mb-4">
                ...
                <div class="col-md-6">
                    <form action="{{ route('products.index') }}" method="GET" class="form-inline d-flex">
                        ...
                        <a href="{{ route('products.index') }}" class="btn btn-secondary ml-2" style="padding: 5px 10px;">
                            Clear
                        </a>
                    </form>
                </div>
                    ...
                    <form action="{{ route('products.index') }}" method="GET" class="form-inline">
                        <div class="form-group mr-2">
                            <select name="status" class="form-control" style="width: 150px;">
                                ...
                                <option value="low" {{ request('status') == 'low' ? 'selected' : '' }}>Low Stock Items</option>
                                <option value="near_expiry" {{ request('status') == 'near_expiry' ? 'selected' : '' }}>Near Expiry Items</option>
                                <option value="out_of_stock" {{ request('status') == 'out_of_stock' ? 'selected' : '' }}>Out of Stock Items</option>
                            </select>
                        </div>
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            @forelse ($products as $product)
                                <tr @if($product->quantity <= 5) class="table-danger" 
                                    @elseif($product->expiry_date && $product->expiry_date->diffInDays(now()) <= 7) class="table-warning" 
                                    @endif>
                                    ...
                                    <td class="text-center">
                                        ...
                                        @endphp
                                        @if ($product->expiry_date && $daysUntilExpiry < 0)
                                            <br>
                                            <span class="badge" style="background-color:  #dc3545; color: white;">Expired</span>
                                        @elseif ($product->expiry_date && $daysUntilExpiry >= 0 && $daysUntilExpiry <= 7)
                                            <br>
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group">
                                            <button class="btn btn-xs btn-info open-edit-modal" data-id="{{ $product->id }}" title="Edit">
